...
// It is the rate at which the time taken increase wrt the input size.
//Rate = Theta
//Graph plotted btw time taken vs input size which is increases linearly at some angle and angle is known as theta. But T.C in computers are not in terms of mins or secs.
//In terms of Big-O notation--> O(Time taken);
//Rules:
//Always compute T.C in terms of worst case scenario.
//Avoid constants.
//Avoid lower values.
//Example:
//for(i=0;i<=N;i++){    //O(N * 3) mult by 3 cuz of repeating steps of checking cond,
// cout<<"Hey";                              printing the statement and updating the 
//return 0;}                                                               statement.
